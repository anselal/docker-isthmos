        ...
        case 'action:stop':
            if (record == null) {
                ...
                w2confirm(msg, "Stop Container", function btn(answer) {
                    ...
                    switch (answer) {
                        case 'Yes':
                            $.get("http://127.0.0.1:5000/docker/container/stop/" + record.Id, function () {
                            })
                                .done(function () {
        ...
        case 'action:remove' :
            if (record == null) {
                ...
                w2confirm(msg, "Remove Container", function btn(answer) {
                    ...
                    switch (answer) {
                        case 'Yes':
                            $.get("http://127.0.0.1:5000/docker/container/remove/" + record.Id, function () {
                            })
                                .done(function () {
...
function onClickContainers(event) {
    ...
    w2ui['container_processes'].load('http://127.0.0.1:5000/docker/container/top/' + record.Id);

    // Load Container Logs
    w2ui['container_logs'].clear();
    w2ui['container_logs'].load('http://127.0.0.1:5000/docker/container/logs/' + record.Id);

    // Get string or JSON data and add it to the bottom of the layout
    /*
     $.get('http://127.0.0.1:5000/docker/container/logs/' + record.Id, function (data, status) {
     w2ui['layout'].content('bottom', "Data: " + data + "\nStatus: " + status);
     });

     $.getJSON('http://127.0.0.1:5000/docker/container/logs/' + record.Id, function (result) {
